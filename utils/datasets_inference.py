...
def create_dataloader(path, video_root_path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
    ...
    with torch_distributed_zero_first(rank):
        #LoadImagesAndLabels
        dataset = LoadClipsAndLabels_my(path, video_root_path, imgsz, batch_size,
...
class LoadClipsAndLabels_my(Dataset):
    ...
    def __init__(self, path, video_root_path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, stride=32, pad=0.0, prefix='', is_training=True, num_frames=5):
        self.img_size = img_size
        ...
        shapes = list(zip(*cache.values()))[0]
        
        #self.labels = list(labels)
        #self.instances = list(instances)
        self.shapes = np.array(shapes, dtype=np.float64).reshape(-1, 2)
        self.img_files = list(cache.keys()) 
        #self.label_files = img2label_paths(cache.keys(), self.annotation_path)  # update
        #print(n, len(self.img_files))
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix='', ):
        ...
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(pool.imap(verify_image_label, zip(self.img_files, repeat(prefix))),
                        desc=desc, total=len(self.img_files))
            for im_file, shape, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
    ...
    def __getitem__(self, index):
        ...
        # Convert
        t:int = len(temporal_frames_path)
        ...
        for tii, tfp in enumerate(temporal_frames_path):
            if os.path.basename(tfp) == main_frame_path:
                main_frameid_id = tii
                break
        #print(main_frame_path, temporal_frames_path, main_frameid_id)
        ...
        return torch.from_numpy(img), temporal_frames_path, shapes, main_frameid_id

    @staticmethod
    def collate_fn(batch):
        img, path, shapes, main_frameid_ids = zip(*batch)  # transposed label - > B [ n X T X 6 ]
        
        main_frame_ids = [] #[fid for fid in main_frameid_ids] #note the principal frameid relative in a batch around which temporal sample is generated
        T = img[0].shape[0]
        B = len(img)
        for i in range(B):
            for ti in range(T):
        ...
        return img, path, shapes, main_frame_ids
    @staticmethod
...
def verify_image_label(args):
    ...
    try:
        ...
        #print(im_file, shape)
        return im_file, shape, nm, nf, ne, nc, msg
    except Exception as e:
        print(e)
        nc = 1
        msg = f'{prefix}WARNING: {im_file}: ignoring corrupt image/label: {e}'
        return [None, None, nm, nf, ne, nc, msg]
