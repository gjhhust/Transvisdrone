...
def create_dataloader(path, annotation_path, video_root_path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
    ...
    dataloader = loader(dataset,
                        ...
                        pin_memory=True,
                        drop_last=False,
                        collate_fn=LoadClipsAndLabels_my.collate_fn4 if quad else LoadClipsAndLabels_my.collate_fn,
                        generator=generator,
                        shuffle = shuffle,
...
def img2label_paths_my(img_paths, annotation_dir):
    #return [os.path.join(annotation_dir, os.path.basename(img_path).replace(".png", ".txt")) for img_path in img_paths ]
    get_video_name = lambda x: os.path.basename(os.path.dirname(x))
    get_ann_name = lambda x: os.path.basename(x).split(".")[0]+".txt"  #to adjust to ishan's annotations
    meta_paths =[ [get_video_name(x), get_ann_name(x)] for x in img_paths]
    #print(meta_paths[0])
    return [ os.path.join(annotation_dir, video_name, ann_name) for (video_name, ann_name) in meta_paths]
...
def get_video_length(video_root_path):
    ...
    if os.path.exists(os.path.join(video_root_path, "video_length_dict.pkl")):
        ...
        for video_dir in videos_path:
            video_name = os.path.basename(video_dir) #video_name
            n_frames = len(os.listdir(video_dir))
            video_id_length[video_name] = n_frames
...
class LoadClipsAndLabels(Dataset):
    ...
    def get_video_length(self, index):
        ...
        video_name = os.path.basename(os.path.dirname(self.img_files[index]))
        return int(self.video_length_dict[video_name])
...
def get_video_length_txt(video_root_path):
    video_id_length = {}
    if os.path.exists(os.path.join(video_root_path, "video_length_dict.pkl")):
        video_id_length = pickle.load(open(os.path.join(video_root_path, "video_length_dict.pkl"), "rb"))
    else:
        # 读取文件内容到列表
        with open(video_root_path, 'r') as file:
            addresses = file.readlines()
            # 移除每行末尾的换行符
            addresses = [address.strip() for address in addresses]
        assert len(addresses) > 0, print(f"{video_root_path} found empty images")
        for frame_path in addresses:
            video_name = os.path.basename(os.path.dirname(frame_path)) #video_name
            if video_name not in video_id_length:
                video_id_length[video_name] = 0
            video_id_length[video_name] += 1
        # for video_path in videos_path:
        #     video_id = int(os.path.basename(video_path).split(".")[0].split("_")[1]) #Clip_2.mov -> 2
        #     cap = cv2.VideoCapture(video_path)
        #     n_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
...
class LoadClipsAndLabels_my(Dataset):
    # YOLOv5 train_loader/val_loader, loads clips and labels for training and validation
    cache_version = 0.6  # dataset labels *.cache version
    ...
    def __init__(self, path, annotation_path, video_root_path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        print(f"Frame wise augmentation set to {self.frame_wise_aug}")
        self.video_root_path = video_root_path
        self.video_length_dict = get_video_length_txt(self.video_root_path)
        self.num_frames = num_frames #int(self.hyp['num_frames'])
        self.skip_frames = int(self.hyp["skip_rate"]) if self.hyp else self.num_frames - 1
        self.albumentations = None
        if augment:
        ...
        if not exists:
            try:
                ...
                with open(video_root_path, 'r') as file:
                    addresses = file.readlines()
                    # 移除每行末尾的换行符
                    self.img_files = [os.path.join(path, address.strip()) for address in addresses]
                assert self.img_files, f'{prefix}No images found'
            except Exception as e:
                raise Exception(f'{prefix}Error loading data from {path}: {e}\nSee {HELP_URL}')
            
            self.label_files = img2label_paths_my(self.img_files, self.annotation_path)  # labels
        ...
        self.shapes = np.array(shapes, dtype=np.float64)
        self.img_files = list(cache.keys()) 
        self.img_files = [os.path.abspath(path) for path in self.img_files]
        self.label_files = img2label_paths_my(cache.keys(), self.annotation_path)  # update
    ...
    def sample_temporal_frames(self, index):
        ...
        if current_frame_id >= max_sample_window+1:
            ...
            sample_frame_ids = [ i  for i in range(current_frame_id, current_frame_id+max_sample_window, skip_frames+1)]
        image_file_parent_path = Path(self.img_files[index]).parents[0]
        sample_frame_paths = [str(Path.joinpath(image_file_parent_path, f"{str(sample_frame_id).zfill(7)}.png")) for sample_frame_id in sample_frame_ids]
        #print(f"Sampled frame ids {sample_frame_ids}, principal frame id {current_frame_id}, clip id {clip_id}")
        sample_frame_ids = [self.img_file_to_indices_mapping[str(img_file_path)] if str(img_file_path) in self.img_file_to_indices_mapping else -1 for img_file_path in sample_frame_paths]
        assert self.img_files[index] in sample_frame_paths, print(f"Temporal Sampling :Principal key frame missing current_frame_path {self.img_files[index]}, sample_frame_paths {sample_frame_paths}, total frames {n_frames}")
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                ...
                l, index = np.unique(l, axis=0, return_index=True)  # remove duplicate rows
                i = i[index]
                if len(l) < nl:
